...
class FilterScenario(BaseScenario):
    ...
    def getItemsPriceAuto(self):
        ...
        if self.base_url != self.driver.current_url:
            ...
            # Step 2: User selects "Price low to high" (or "Price high to low") option between filter options
            selected_option_elem = list(filter(lambda x: x.get_attribute("value") == "lohi", option_filter_elems))[
                0] if not self.reverse else list(filter(lambda x: x.get_attribute("value") == "hilo", option_filter_elems))[0]
            selected_option_elem.click()

            # Step 3: System loads page with product cards sorted by price in crescent order (or decrescent order)
            item_elems = self.driver.find_elements(
                by=By.CLASS_NAME, value="inventory_item")
    ...
    def sortItemByPrice(item_elem):
        ...
        price_item_elem = item_elem.find_element(
            by=By.CLASS_NAME, value="inventory_item_price")
    ...
    def comparePriceWithExpectedResult(self):
        ...
        for i in range(len(getted_item_elems)):
            ...
            getted_price_item_elem = getted_item_elems[i].find_element(
                by=By.CLASS_NAME, value="inventory_item_price")
            expected_price_item_elem = expected_item_elems[i].find_element(
                by=By.CLASS_NAME, value="inventory_item_price")
